...
async def next_page(bot, query):
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("☜︎︎︎ ʙᴀᴄᴋ", callback_data=f"next_{req}_{key}_{off_set}"),
        ...
        btn.append(
            [
                InlineKeyboardButton("☜︎ ʙᴀᴄᴋ", callback_data=f"next_{req}_{key}_{off_set}"),
                InlineKeyboardButton(f"🗓 {round(int(offset) / 10) + 1} / {round(total / 10)}", callback_data="pages"),
                InlineKeyboardButton("ɴᴇxᴛ➪", callback_data=f"next_{req}_{key}_{n_offset}")
            ],
        )
    ...
    if query.data.startswith("file"):
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                await query.answer('ᴄʜᴇᴄᴋ,ᴘᴍ ɪ ʜᴀᴠᴇ sᴇɴᴛ ғɪʟᴇs', show_alert=True)
        ...
        buttons = [[
            InlineKeyboardButton('ᴍᴏᴠɪᴇ ʀᴇǫ', url='https://t.me/TeamEvamaria'),
